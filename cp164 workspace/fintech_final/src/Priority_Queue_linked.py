...
class Priority_Queue:
    ...
    def _move_node(self, node):
        ...
        if self._front is None:
            ...
            # Check to see if node belongs at the front
            if node._value <= self._front._value:
                node._next = self._front
                self._front = node

            # Check to see if node belongs at the rear
            elif node._value >= self._rear._value:
                self._rear._next = node
                self._rear = node

            # If node belongs somewhere in the the middle of the front and rear
            else:
                previous = None
                curr = self._front
                while node._value > curr._value:
                    previous = curr
                    curr = curr._next
