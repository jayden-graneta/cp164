...
class BST:
    ...
    def are_siblings(self, v1, v2):
        ...
        if self._root is not None:
            ...
            if v1 < v2:
                ...
                siblings = self._are_siblings_aux(self._root, v2, v1)
    ...
    def _are_siblings_aux(self, parent, v1, v2):
        ...
        if parent._right is not None and parent._left is not None:
            if parent._left._value == v1 and parent._right._value == v2:
                siblings = True

        if not siblings and parent._left is not None:
            siblings = self._are_siblings_aux(parent._left, v1, v2)

        if not siblings and parent._right is not None:
    ...
    def _longest_path_aux(self, node, path):
        """
        ---------------------------------------------------------
        ...
        if node is not None:
            ...
            # Go to the node that has the higher height
            lh = self._node_height(node._left)
            rh = self._node_height(node._right)

            if lh >= rh:
                path = self._longest_path_aux(node._left, path)
            else:
                path = self._longest_path_aux(node._right, path)
