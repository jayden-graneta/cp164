...
class Sorts:
    ...
    @staticmethod
    def gnome_sort(deque):
        """
        -------------------------------------------------------
        Sorts a Deque using the Gnome Sort algorithm.
        Use: gnome_sort(deque)
        -------------------------------------------------------
        Parameters:
            deque - a linked structure of comparable elements (Deque)
        Returns:
            None
        -------------------------------------------------------
        """
        gnome = deque._front._next if deque._front is not None else None

        while gnome is not None:
            if gnome._value >= gnome._prev._value:
                gnome = gnome._next
            else:
                deque._swap(gnome, gnome._prev)
                if gnome == deque._front:
                    gnome = gnome._next
