...
class BST:
    ...
    def total_height(self):
        ...
        """
        node = self._root
        total = self._total_height_aux(node)
    ...
    def _total_height_aux(self, node):
        ...
        if node is None:
            ...
            total = 1 + node._height + self._total_height_aux(node._right)
        elif node._right is not None and node._right is None:
            total = 1 + node._height + self._total_height_aux(node._left)
        else:
            total = node._height + self._total_height_aux(
                node._left) + self._total_height_aux(node._right)
